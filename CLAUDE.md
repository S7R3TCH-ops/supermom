...
## Current version: 0.13.43 — Aug 15, 2026 (LIVE)
...
- **v0.13.43** (Aug 15) — keyboard covering active field in NewClientSheet, reported via a WhatsApp screenshot Joel dropped in `second-brain/00-inbox/mobile-sync/`. `useKeyboardFocus`'s trailing 260px spacer wasn't enough to keep the focused input clear of the iOS keyboard on longer forms (Add Client's Street/City/Postal Code fields). Fix: `<form>` in `NewClientSheet.jsx` now has an `onFocusCapture` handler that calls `scrollIntoView({block:'center', behavior:'smooth'})` on the focused input/textarea/select, 300ms after focus (keyboard-animation delay). Pushed at Joel's explicit go-ahead **before** his usual phone-test pass — flag if it recurs. Build clean, Vitest 111/111.
