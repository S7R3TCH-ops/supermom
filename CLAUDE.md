...
## Current version: 0.13.20 — Jul 16, 2026 (package.json synced)

Sandra's business is live — data wiped and re-provisioned Jun 9. App in active use.

**Deploy status: v0.13.20 is committed to main but NOT yet deployed** — v0.13.19 (`dpl_D3j6b6pbd5QxapSU2yq4AChvmhvR`) is what's live in production. v0.13.20 is the Jul 16 Clients/Finance UX scrutiny pass + invoice same-named-job clarity fix (two commits, details below). Deploying is Joel's call (remember the don't-rapid-redeploy cron rule). The Finance hero/stat-grid restructure is the one change worth Joel + Sandra eyeballing on first deploy — revenue drill-in moved from a stat card into the (now tappable) hero.
