...
## Current version: 0.12.36 — committed Jun 13, 2026
...
- **v0.12.36 (Jun 13)** — Invoice layout pass (partial — more work needed next session). (1) "Other Outstanding Balances" section redesigned: renamed to "Also Outstanding for This Client" with descriptor subline, 2px top border separates it from invoice totals, owing amounts now red (#DC2626) instead of bold black, combined total bar switched from cream to amber (#FEF3C7) with red amount at larger size — fixes inverted color hierarchy where the biggest owed number was the least prominent. (2) Client street address now shown on its own line in "Issued To" block on both web (InvoiceView.jsx) and PDF (invoicePdf.js). Both renderers updated in sync. ⚠️ Invoice layout still needs more work — Sandra found it confusing; not fully resolved yet. Pick up next session.
...
1. **⚠️ Invoice layout (in progress, not resolved)** — v0.12.36 improved the "Other Outstanding" section color hierarchy and added client street address, but Sandra still finds it confusing overall. Need to actually render an invoice with multiple outstanding balances and review the full layout top-to-bottom. Consider `/impeccable` critique pass on the web view. Both `InvoiceView.jsx` (web) and `api/_lib/invoicePdf.js` (PDF) must stay in sync — any change needs to land in both.
2. **⚠️ Device verification** — v0.12.32 Android perf fix (bog-down + Sandra-view freeze on Pixel) and all Jun 12 fixes not yet phone-tested. Do these first.
3. **GCal sync for new jobs** — `triggerGCalSync` is fire-and-forget with no response checking so failures are completely silent. Most likely cause: Google OAuth app is in "Testing" mode → refresh tokens expire after 7 days. Sandra needs to reconnect (Settings → Google Calendar → CONNECT). If token is valid and sync still fails, add logging to surface what `api/sync/gcal.js` is returning. The `syncUpcomingJobs()` call on reconnect in Settings.jsx should backfill once she reconnects.
3. **Home.jsx drive-time + background-resume bugs (diagnosed, not yet fixed)** — Two issues, two separate commits:
...
4. **Design system critique (in progress)** — Next: **Client profile** → Job detail → `$impeccable document` to refresh DESIGN.md.
5. **AI chat interface** — `api/ai/[action].js` + `ANTHROPIC_API_KEY` already in place. Needs chat UI + convo state. HIGH PRIORITY.
