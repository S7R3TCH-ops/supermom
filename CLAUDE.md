...
## Current version: 0.13.60 — Sep 14, 2026 (pushed/live)
...
- **v0.13.59** (Sep 14) — invoice/receipt job line items were sorting in DB insertion order instead of date order, reported by Joel. `decorateInvoiceWithBalances()` in [src/lib/invoiceBalances.ts](src/lib/invoiceBalances.ts) fetched a client's completed jobs with no `.order()` clause, so Supabase/PostgREST returned rows in default (creation) order — confirmed live against real client data, order was scrambled vs service date. This one query feeds `invoiceJobBalances`, `otherOutstanding`, and `alsoPaid`; both `InvoiceView.jsx` (web) and `api/_lib/invoicePdf.ts` (PDF/receipt) just `.map()` over those arrays with no re-sort, so one fix covers invoice, receipt, and outstanding-jobs sections everywhere. Fix: added `.order('scheduled_date', { ascending: true })` — jobs now list oldest-to-newest, normal invoice reading order. Build clean, Vitest 117/117. **Incomplete — see v0.13.60**, this fix never touched what the PDF/web actually render.
