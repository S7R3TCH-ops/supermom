...
## Current version: 0.12.88 — Jun 17, 2026 (package.json synced)
...
- **v0.12.88 (Jun 17)** — Bundle optimization: (1) All 7 sheet providers (`PostJobSheet`, `NewJobSheet`, `JobDetailSheet`, `NewClientSheet`, `EditClientSheet`, `FinanceDetailSheet`, `AiChatSheet`) changed from static imports to `lazy()` + `Suspense` — sheets now only download when opened. (2) `vite.config.js` adds `manualChunks` vendor split (React + ReactDOM + React Router + TanStack Query → `vendor` chunk). (3) `index.html` adds `mobile-web-app-capable` meta tag. Result: main `index` chunk 487KB → 54KB (89% reduction); vendor chunk (264KB) cached across deploys by SW.
