...
## Current version: 0.13.24 — Jul 17, 2026 (package.json synced)

Sandra's business is live — data wiped and re-provisioned Jun 9. App in active use.

**Deploy status: v0.13.22 is LIVE in prod** (v0.13.20/21/22 all deployed + smoke-tested clean 2026-07-16 — see second-brain status.md). **v0.13.23 committed to main but NOT yet deployed** — it's two device-level mobile fixes (swipe-close + universal viewport fit) that **must be phone-tested on Pixel (both nav modes) + Sandra's iPhone before deploy** (code-verified only: build clean, Vitest 81/81). Deploying is Joel's call (remember the don't-rapid-redeploy cron rule).
...
- **v0.13.24 (Jul 17) — Home job card not auto-refreshing after mark-complete/mark-paid (Joel, mobile capture Jul 17 13:19).** `notifyDataChanged()` (`src/data/events.js`) only scheduled cache invalidation 300ms out via a debounce timer — nothing guaranteed that timer fired before a sheet closed and the user navigated back to Home, a real race window (verified by code reading: query wiring itself is correct, no stale local snapshot in `Home.jsx`, and `sw.js` already `NetworkOnly`s all Supabase calls so it isn't SW caching). Not phone-confirmed — no device/QA-login access this session — so treat as best-evidence fix, not a closed loop. New `notifyDataChangedNow()` (same invalidation, no debounce) wired into `PostJobSheet.jsx`'s two write points (payment/completion save, bundle-settle) — the actual mark-complete/mark-paid path. Debounced version left as-is everywhere else. Build clean, Vitest 81/81. **Needs phone test** alongside the still-pending v0.13.23 gate below.
- **v0.13.23 (Jul 17) — two device-level mobile fixes: swipe-close over-sensitivity + universal viewport/safe-area fit (Joel's 2 priorities for the day). Code-verified only, NOT yet phone-tested.**
