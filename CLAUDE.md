...
## Current version: 0.13.58 — Sep 14, 2026 (committed, not yet pushed)
...
- **v0.13.58** (Sep 14) — F4 shipped, last item in the F1-F4 keyboard/viewport series. Pinned `interactive-widget=resizes-visual` explicitly on `index.html`'s viewport meta tag, and added a comment above `getKeyboardInset()` in `src/lib/appHeight.js` naming the dependency — the whole F1-F3 fix chain relies on `window.innerHeight` staying fixed while only `visualViewport.height` shrinks when the keyboard opens (Chrome's current default behavior); if a future browser default ever switches to `resizes-content`, that delta collapses to ~0 and the void bug returns with no code change to blame. Mechanical, 1 file + 1 comment, pre-cleared for change-auditor per the 2026-08-16 audit (F1/F3/F4 didn't need one individually). Build clean, Vitest 117/117. **Ships alone, own commit, own attribution window** per the original sequencing call — after 7 failed attempts on this bug class historically. **Needs its own device confirm after pushing** — don't skip it because the diff is tiny. Not yet pushed — awaiting Joel's go-ahead.
