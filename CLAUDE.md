...
## Current version: 0.13.76 — Sep 22, 2026 (pending push)

**v0.13.76 (this session)**: Anthropic→Gemini AI provider swap — see Bugs section above for full detail. `api/ai/[action].js` and `api/ai/chat.js` ported from `@anthropic-ai/sdk` to `@google/genai` (new shared helper `api/_lib/gemini.js`, model `gemini-3.6-flash` with thinking disabled). Confirmed live against the deployed Vercel app.
