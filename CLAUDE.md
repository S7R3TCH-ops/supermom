...
## Current version: 0.13.59 — Sep 14, 2026 (committed, NOT pushed — pending change-auditor + Joel go-ahead)
...
- **v0.13.59 (not pushed) — client job-notes "more in my face," v1.** Sandra's complaint: `jobs.job_notes`/`completion_notes` rendered as small italic muted text, easy to miss (`clients.notes` confirmed dead in a prior diagnostic-only session, not touched). Restyled to a high-contrast pink-tinted callout (`T.pinkTint` bg, `1.5px solid T.pink` border, `✦` label matching the app's existing section-label convention, body text bumped from `T.inkSub`/9-10.5px italic to `T.ink` 500-weight 11-13px) in 3 places: `JobDetailSheet.jsx` (Pre-job/Post-Job Notes, was a plain `InfoCard`), `JobCard.jsx` (Home/Schedule job card, `j.notes` = `job_notes` via `selectors.ts:332` — confirmed, `completion_notes` not surfaced here), `UpcomingCard.jsx` (Home's Upcoming card, same `j.notes` field, same bug, found while implementing — not in the original 2-file scope note but identical pattern). `JobCard.jsx`'s owing-status cards (unpaid/overdue/partial) render on a solid dark pill-color fill, not the light page bg, so that variant uses a translucent white overlay (`rgba(255,255,255,0.15)` bg / `rgba(255,255,255,0.35)` border / white text) instead of `T.pinkTint`, mirroring the file's existing `isOwing` color-branch pattern — `T.pinkTint` alone would've gone invisible on those cards. No schema change, no new component, existing `InfoCard` left untouched for its other 8+ callers. Build clean, Vitest 117/117. **Committed to worktree branch `worktree-client-notes-callout`, NOT pushed** — needs the change-auditor pass called for in the original task note before merge/push, and Joel's explicit go-ahead per the standing production-push rule.
