...
const s = StyleSheet.create({
  ...
  infoCol:   { flex: 1, lineHeight: 1.5 },
  infoLabel: { fontSize: 7, fontFamily: 'Helvetica-Bold', color: LABEL_C, letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 5 },
  infoBold:  { fontFamily: 'Helvetica-Bold', color: INK, marginBottom: 2, fontSize: 10 },
  infoMuted: { color: MUTED, marginBottom: 2, fontSize: 10 },
  infoLight: { color: LIGHT, marginBottom: 2, fontSize: 10 },
  // Invoice meta rows
  ...
  footerText:   { fontSize: 10, color: '#444', lineHeight: 1.7 },
    ...
    el(Page, { size: 'A4', style: s.page },
      ...
      V({ style: s.infoRow },
        ...
        V({ style: s.infoCol },
          ...
          clientCity ? T({ style: s.infoMuted }, clientCity) : null,
          client.email ? T({ style: s.infoLight }, client.email) : null,
          client.phone ? T({ style: s.infoLight }, client.phone) : null,
        ),
        // From
        V({ style: s.infoCol },
          T({ style: s.infoLabel }, 'From'),
          T({ style: s.infoBold }, biz.name || 'Supermom for Hire'),
          bizCity ? T({ style: s.infoMuted }, bizCity) : null,
          biz.phone ? T({ style: s.infoLight }, biz.phone) : null,
          biz.email ? T({ style: s.infoLight }, biz.email) : null,
        ),
        // Invoice meta
        V({ style: [s.infoCol, { alignItems: 'flex-end' }] },
          ...
          V({ style: s.metaRow }, T({ style: s.metaKey }, 'DUE'),  T({ style: s.metaVal }, formatDate(invoice.due_date))),
        ),
      ),
      ...
      ...f.activeCosts.map((item, idx) =>
        V({ key: idx, style: s.tableRowAlt },
          V({ style: s.cDate  }, T({ style: s.tdMuted  }, '—')),
          V({ style: s.cDesc  },
            T({ style: s.tdPink   }, 'Additional Cost'),
            T({ style: s.tdMuted  }, item.description || 'Miscellaneous'),
          ),
          V({ style: s.cRate  }, T({ style: s.tdCenter }, '—')),
          V({ style: s.cHours }, T({ style: s.tdCenter }, '—')),
