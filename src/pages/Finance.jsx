    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: T.bg, color: T.ink }}>
      ...
      <div className="sm-scroll" style={{ flex: 1, overflowY: 'auto', padding: '11px 13px 8px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 11 }}>
          {[
            { label: 'Collected',    val: `$${collected.toFixed(0)}`,       sub: `This ${period.toLowerCase()}`,         color: '#22C55E', bg: mode === 'dark' ? 'rgba(34,197,94,0.1)'  : '#F0FFF5', border: mode === 'dark' ? 'rgba(34,197,94,0.4)'  : '#86EFAC', onClick: () => openDetail(`Collected (${period})`, collectedJobs, 'jobs') },
            { label: 'Outstanding',  val: `$${outstanding.toFixed(0)}`,     sub: `${outstandingCount} unpaid`,           color: '#E91E6A', bg: mode === 'dark' ? 'rgba(233,30,106,0.1)' : '#FFF0F7', border: mode === 'dark' ? 'rgba(233,30,106,0.4)' : '#FFD6E8', action: outstandingCount > 0 ? 'Nudge all' : null, onAction: (e) => { e.stopPropagation(); setShowNudges(true); }, onClick: () => openDetail('Outstanding', outstandingJobs, 'jobs') },
            { label: 'Expenses',     val: `$${periodExpenses.toFixed(0)}`,  sub: `This ${period.toLowerCase()}`,         color: '#F59E0B', bg: mode === 'dark' ? 'rgba(245,158,11,0.1)' : '#FFFBEB', border: mode === 'dark' ? 'rgba(245,158,11,0.45)' : '#FCD34D', action: '+ Add', onAction: (e) => { e.stopPropagation(); setShowNewExpense(true); }, onClick: () => openDetail(`Expenses (${period})`, periodExpList, 'expenses') },
